...
Spectracer 是一款面向音乐采谱（扒谱）场景的现代音频频谱分析与编辑工具，当前仓库已进入 **GUI Alpha + 可视化强化** 阶段，并准备推进到 **MIDI 编辑/试听** 环节。
...
接下来准备进入 **MIDI 相关环节**，重点包括：

- MIDI 覆盖层音符轨与更完整的编辑流程
- MIDI 导出
- 循环区间、项目保存/加载，以及更完整的 BPM / 拍号事件编辑

更详细的阶段计划见：`plans/spectracer.md`
...
1. MIDI 试听链路（点击热图/钢琴键发声）
2. MIDI Note 数据模型与覆盖层显示
3. 基础编辑操作（新增、移动、拉伸、删除）
4. MIDI 导出与最小项目闭环

如果你想了解当前阶段拆解与后续里程碑，请直接查看 `plans/spectracer.md`。
